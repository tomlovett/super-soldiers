import type { Mission } from '../../app/types/types'
...
const MissionCard = ({ name }: Mission) => (
	<li className="flex justify-between gap-x-6 py-5">
		<div className="flex min-w-0 gap-x-4">
			...
			<p className="mt-1 text-xs leading-5 text-slate-500">Last seen <time>3h ago</time></p>
		</div>
	</li>
)
...
const MissionPage = () => (
	<div className="container mx-auto px-8 items-center justify-between p-12">
		<ul role="list" className="column divide-y divide-gray-400">
			{missionsList.map(mission => <MissionCard name={mission.name} key={mission.name}/> )}
		</ul>
	</div>
)
